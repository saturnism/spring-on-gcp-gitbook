...
Publish Java artifacts to Artifact Registry, which can host Maven repositories.  See [Artifact Repository](../deployment/artifact-repository.md) section.

### Cloud Trace

Use Spring Cloud Sleuth and send Distributed Tracing data to Cloud Trace, using Spring Cloud GCP.  See [Trace section](cloud-services/trace.md).

### Cloud Logging

Aggregate logs into a centralized logging console to easily search and view logs. See [Logging section](cloud-services/logging.md).

### Error Reporting

Automatically identifies Java exceptions and produce reports. Easily see new exceptions and their frequencies. See [Logging section](cloud-services/logging.md#error-reporting).

### Cloud Monitoring

Collect system and application metrics, build dashboards, and setup alerts.  See [Metrics section](cloud-services/metrics.md).

### Cloud Debugger

Cloud Debugger can debug your production application without halting the application. Cloud Debugger can capture application state as a Snapshot, and also able to add additional log messages without redeploying the code. See [Cloud Debugger section](cloud-services/debugging.md).

### Cloud Profiler

Cloud Profiler can continuously profile CPU and heap usages in a production application with minimal overhead.  The profiled flame graph can help you understand performance hotspots. See [Cloud Profiler section](cloud-services/profiling.md).
